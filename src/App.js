...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.setStatus, 1000)
  }
  ...
  OnClickImage = thumbnailUrl => {
    ...
    const imageValue = imagesList.filter(
      eachOne => eachOne.thumbnailUrl === thumbnailUrl,
    )
  ...
  render() {
    const {category, isTrue, score, imgUrl, time} = this.state
    const filteredList = imagesList.filter(
      eachObject => eachObject.category === category,
    )
    return (
        ...
        <div>
          {!isTrue && (
            <div className="total-card">
              <ul>
                <li className="list-item">
                  <img src={imgUrl} alt="match" className="show-img" />
                </li>
              </ul>

              <ul className="tab-adj">
                {tabsList.map(each => (
                  <li key={each.tabId} id={each.tabId} className="tab-list">
                    <button
                      type="button"
                      className={`tab-btn ${
                        category === each.tabId ? 'highlight' : ''
                      }`}
              ...
              <ul className="imgs-sty">
                {filteredList.map(eachImage => (
                  <li key={eachImage.id} id={eachImage.id} className="img-list">
                    <button
                      className="img-btn"
                      type="button"
                      onClick={() => this.OnClickImage(eachImage.thumbnailUrl)}
                    >
                      <img
